...
class Interpreter:
    ...
    def step(self):
        ...
        if self.debug:
            print
            '{:03}:{} {}'.format(self.ip, opcode, tokens)
    ...
    def find_in_dict(self, args):
        ...
        if self.debug:
            ...
            ele, type(ele)
        try:
            ele = self.run_stack[self.ep].symbolDict[ele]
            return ele
        except Exception:
            pass
        try:
        ...
        if self.debug:
            print
            '{} - {}'.format(ele, "Not found in the dictionary")

    def RUN(self):
        while self.ip < len(self.program):
            self.step()

    def takein(self, args):
        par = input()
        self.run_stack[self.ep].symbolTable.append(par)
    ...
    def CALL(self, args):
        ...
        i = 0
        while i < no_pars:
            list_pars.insert(0, self.find_in_dict(self.run_stack[self.ep].symbolTable.pop()))
            i += 1
        self.ip = f_line
        runtime = Runtime()
        ...
        while i < no_pars:
            ...
            tokens = line.split()
            opcode = tokens.pop(0)
            self.run_stack[self.ep].symbolDict[tokens[0]] = new_par
            self.ip += 1
            i += 1

    def PRINT(self, args):
        par = self.run_stack[self.ep].symbolTable.pop()
        op = self.find_in_dict(par)
        print
        op

    def PUSH(self, args):
        self.run_stack[self.ep].symbolTable.append(args)

    def STORE(self, args):
        par = self.run_stack[self.ep].symbolTable.pop()
        par = self.find_in_dict(par)
        self.run_stack[self.ep].symbolDict[args[0]] = par

    def ADD(self, args):
        par1 = self.run_stack[self.ep].symbolTable.pop()
        par2 = self.run_stack[self.ep].symbolTable.pop()
        op1 = self.find_in_dict(par1)
        op2 = self.find_in_dict(par2)
        self.run_stack[self.ep].symbolTable.append(op1 + op2)
    ...
    def SUBTRACT(self, args):
        ...
        op1 = self.find_in_dict(par1)
        op2 = self.find_in_dict(par2)
        self.run_stack[self.ep].symbolTable.append(op2 - op1)
    ...
    def AND(self, args):
        ...
        op1 = self.find_in_dict(par1)
        op2 = self.find_in_dict(par2)
        self.run_stack[self.ep].symbolTable.append(op2 and op1)
    ...
    def OR(self, args):
        ...
        op1 = self.find_in_dict(par1)
        op2 = self.find_in_dict(par2)
        self.run_stack[self.ep].symbolTable.append(op2 or op1)
    ...
    def STACK(self, args):
        self.run_stack[self.ep].symbolDict[args[0]] = []

    def STACKPUSH(self, args):
        par = self.run_stack[self.ep].symbolTable.pop()
        st = self.run_stack[self.ep].symbolDict[args[0]]
        st.append(par)
        self.run_stack[self.ep].symbolDict[args[0]] = st
    ...
    def STACKISEMPTY(self, args):
        st = self.run_stack[self.ep].symbolDict[args[0]]
        if len(st) == 0:
            self.run_stack[self.ep].symbolTable.append(True)
        else:
            self.run_stack[self.ep].symbolTable.append(False)
